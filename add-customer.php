...
if(isset($_POST['create_customer']))
{
   ...
   //sql to inset the values to the database
    $query="insert into customer (fname, lname, email, mobile, pic, bank_details, passport, fb_id, dob, address, c_type, sex, status) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt = $mysqli->prepare($query);
    //bind the submitted values with the matching columns in the database.
    $rc=$stmt->bind_param('sssssssssssss', $fname, $lname, $email, $mobile, $pic, $bank_details, $passport, $fb_id, $dob, $address, $c_type, $sex, $status);
    $stmt->execute();
    
    $msg = "Customer Details Added!";
                        ...
                        <div class="panel-body">
                           <form class="col-sm-12" method="POST"  enctype="multipart/form-data" >
                              ...
                              <div class="form-group">
                                 ...
                                 <input type="text" class="form-control" name="bank_details" placeholder="Enter Bank Account Number" required>
                              </div>
                              <div class="form-group">
                                 <label>Passport</label>
                                 <input type="text" class="form-control" name="passport" placeholder="Enter Passport details" required>
                              </div>
                              <div class="form-group">
                                 <label>Facebook Id</label>
                                 <input type="text" class="form-control" name="fb_id" placeholder="Enter Facebook details" required>
                              </div>
                              <div class="form-group">
                                 <label>Date of Birth</label>
                                 <input id='minMaxExample' type="text" name="dob" class="form-control" placeholder="Enter Date...">
                              </div>
                              <div class="form-group">
                                 <label>Address</label>
                              ...
                              <div class="form-check">
                                 ...
                                 <label class="radio-inline"><input  type="radio" name="status" value="Inactive" >Inctive</label>
                              </div>
                              <div class="reset-button">
                                 
                                 <input type="submit" name="create_customer" value="Add Customer" class="btn btn-success">
                              </div>
                           </form>
                        </div>
                     </div>
